...
const CategoryCardItem = ({ cat, idx, onClick }) => {
  ...
  return (
    <motion.div
      ...
      className="w-full flex flex-col items-center cursor-pointer group text-center px-1"
      onClick={() => onClick(cat)}
    >
      <div className={`relative w-28 h-28 sm:w-36 sm:h-36 md:w-48 md:h-48 rounded-full overflow-hidden shadow-sm group-hover:shadow-xl transition-all duration-300 border-[3px] border-transparent group-hover:border-[#be9456]/40 group-hover:scale-105 ${cat.isContain ? 'bg-white p-1' : ''}`}>
        {!imgLoaded && (
      ...
      <h3 className="mt-3 md:mt-5 font-playfair text-[13px] sm:text-[15px] md:text-[18px] font-medium text-[#3a3532] group-hover:text-[#b58953] transition-colors leading-tight max-w-[130px] sm:max-w-none">
        {cat.title}
      </h3>
    </motion.div>
  );
...
const CategoryCards = () => {
  ...
  return (
    <section className="w-full pt-4 pb-32 md:py-24 bg-transparent relative overflow-hidden">
      <div className="max-w-6xl mx-auto px-3 sm:px-6">
        ...
        >
          ...
          <h2 className="font-playfair text-2xl sm:text-3xl md:text-5xl font-medium mb-2 md:mb-3 tracking-tight text-[#2b2724]">
            Shop by Occasion
          </h2>
        </motion.div>

        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4 sm:gap-6 md:gap-12 w-full place-items-center">
